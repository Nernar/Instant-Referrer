...
public class LibraryRegistry {
    ...
    private static Library resolveDependencyInList(Collection<Library> libs, LibraryDependency dependency) {
        ...
        for (Library library : libs) {
            if (!library.isInvalid() && dependency.isMatchesLib(library) && (result == null || result.getVersionCode() < library.getVersionCode())) {
                result = library;
            }
        }
        return result;
    ...
    public static Library resolveDependency(LibraryDependency dependency) {
        ...
        if (lib != null) {
            if (lib.isShared()) {
                return resolveSharedDependency(dependency);
            }
            return lib;
        }
        return resolveSharedDependency(dependency);
    ...
    private static void importLibraryInternal(Scriptable scope, Library library, LibraryDependency dependency, String exportName) {
        ...
        LibraryExport export2 = library.getExportForDependency(dependency, exportName);
        if (export2 != null) {
            scope.put(export2.name, scope, export2.value);
        } else {
