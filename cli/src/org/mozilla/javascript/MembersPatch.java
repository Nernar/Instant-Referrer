...
public class MembersPatch {
    ...
    static Method override(Class<?> cl, Method method) {
        String path = String.valueOf(cl.getName()) + "." + method.getName();
        if (overrides.containsKey(path)) {
            ...
            try {
                ...
                Method overridenMethod = overridenClass.getMethod(overridenPath.substring(ind + 1), method.getParameterTypes());
                ...
                Logger.debug("MEMBERS_PATCH", "Could't override method " + path + " with arguments types " + Arrays.toString(method.getParameterTypes()));
            }
        }
        return method;
    }
